...
if [ -z $LATEST_PMA ]; then
    LATEST_PMA=$(curl -s http://lampd.googlecode.com/svn/src/pmalist.txt | tail -1 | awk -F- '{print $2}')
fi
...
if [[ "$UPGRADE_PHP" = "y" || "$UPGRADE_PHP" = "Y" ]];then
    ...
    if [ ! -s php-$LATEST_PHP.tar.gz ]; then
        ...
        BACKUP_PHP_LINK="http://lampd.googlecode.com/svn/src/php-${LATEST_PHP}.tar.gz"
...
if [[ "$UPGRADE_PMA" = "y" || "$UPGRADE_PMA" = "Y" ]];then
    ...
    if [ ! -s phpMyAdmin-$LATEST_PMA-all-languages.tar.gz ]; then
        ...
        BACKUP_PMA_LINK="http://lampd.googlecode.com/svn/src/phpMyAdmin-${LATEST_PMA}-all-languages.tar.gz"
