...
#   Author: a950216t <a950216t@gmail.com>
#   Intro:  http://lamp.myxnova.com/
#===============================================================================================

clear
echo "#############################################################"
echo "# LAMP Auto Install Script for CentOS / RedHat / Fedora"
echo "# Intro: http://lamp.myxnova.com/"
echo "#"
echo "# Author: a950216t <a950216t@gmail.com>"
echo "#"
echo "#############################################################"
echo ""
...
function download_file(){
    if [ -s $1 ]; then
        ...
        if ! wget -c http://lampd.googlecode.com/svn/src/$1;then
...
function install_apache(){
    if [ ! -d /usr/local/apache/bin ];then
        ...
        make install
        if [ $? -ne 0 ]; then
            echo "Installing Apache failed, Please visit http://lamp.myxnova.com/ and contact."
            exit 1
        fi
		...
		cd $cur_dir
		wget -c http://www.yahei.net/tz/tz_tw.zip
		unzip -q tz_tw.zip -d /data/www/default
		wget --no-check-certificate https://github.com/phpsysinfo/phpsysinfo/archive/v3.2.0.tar.gz
		tar zxvf v3.2.0.tar.gz
		mv phpsysinfo-3.2.0 /data/www/default/phpsysinfo
...
function install_mariadb(){
    if [ ! -d /usr/local/mariadb ];then
        ...
        make install
        if [ $? -ne 0 ]; then
            echo "Installing MariaDB failed, Please visit http://lamp.myxnova.com/ and contact."
            exit 1
        fi
...
function install_mysql(){
    if [ ! -d /usr/local/mysql ];then
        ...
        make install
        if [ $? -ne 0 ]; then
            echo "Installing MySQL failed, Please visit http://lamp.myxnova.com/ and contact."
            exit 1
        fi
...
function install_php(){
    if [ ! -d /usr/local/php ];then
        ...
        --enable-zip 
        if [ $? -ne 0 ]; then
            echo "PHP configure failed, Please visit http://lamp.myxnova.com/ and contact."
            exit 1
        fi
        make && make install
        if [ $? -ne 0 ]; then
            echo "Installing PHP failed, Please visit http://lamp.myxnova.com/ and contact."
            exit 1
        fi
...
function install_cleanup(){
    ...
    if [ $? -ne 0 ]; then
        echo "Apache Starting failed, Please visit http://lamp.myxnova.com/ and contact."
        exit 1
    fi
    ...
    if [ -s /usr/local/apache ] && [ -s /usr/local/php ] && [ -s /usr/local/mysql -o -s /usr/local/mariadb ]; then
        ...
        echo "Welcome to visit http://lamp.myxnova.com/"
        echo "Enjoy it!"
        echo ""
    else
        echo ""
        echo 'Sorry, Failed to install LAMP!';
        echo 'Please visit http://lamp.myxnova.com/ and contact.';
    fi
}
