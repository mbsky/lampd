...
function sys_linux()
{
    ...
    $res['memRealUsed'] = $res['memTotal'] - $res['memFree'] - $res['memCached'] - $res['memBuffers']; //真實內存使用
    $res['memRealFree'] = $res['memTotal'] - $res['memRealUsed']; //真實空閒
    $res['memRealPercent'] = (floatval($res['memTotal'])!=0)?round($res['memRealUsed']/$res['memTotal']*100,2):0; //真實內存使用率
    $res['memCachedPercent'] = (floatval($res['memCached'])!=0)?round($res['memCached']/$res['memTotal']*100,2):0; //Cached內存使用率
...
$uptime = $sysInfo['uptime']; //在線時間
$stime = date('Y-m-d H:i:s'); //系統當前時間

//硬盤
$dt = round(@disk_total_space(".")/(1024*1024*1024),3); //總
$df = round(@disk_free_space(".")/(1024*1024*1024),3); //可用
$du = $dt-$df; //已用
$hdPercent = (floatval($dt)!=0)?round($du/$dt*100,2):0;
$load = $sysInfo['loadAvg'];    //系統負載

//判斷內存如果小於1G，就顯示M，否則顯示G單位
if($sysInfo['memTotal']<1024)
{
    ...
    $mc = $sysInfo['memCached']." M";    //cache化內存
    $mb = $sysInfo['memBuffers']." M";    //緩衝
    $st = $sysInfo['swapTotal']." M";
    $su = $sysInfo['swapUsed']." M";
    $sf = $sysInfo['swapFree']." M";
    $swapPercent = $sysInfo['swapPercent'];
    $memRealUsed = $sysInfo['memRealUsed']." M"; //真實內存使用
    $memRealFree = $sysInfo['memRealFree']." M"; //真實內存空閒
    $memRealPercent = $sysInfo['memRealPercent']; //真實內存使用比率
    $memPercent = $sysInfo['memPercent']; //內存總使用率
    $memCachedPercent = $sysInfo['memCachedPercent']; //cache內存使用率
}
else
{
    ...
    $swapPercent = $sysInfo['swapPercent'];
    $memRealUsed = round($sysInfo['memRealUsed']/1024,3)." G"; //真實內存使用
    $memRealFree = round($sysInfo['memRealFree']/1024,3)." G"; //真實內存空閒
    $memRealPercent = $sysInfo['memRealPercent']; //真實內存使用比率
    $memPercent = $sysInfo['memPercent']; //內存總使用率
    $memCachedPercent = $sysInfo['memCachedPercent']; //cache內存使用率
}
...
<div id="page">
    <table>
        <tr>
            <th class="w_logo">PHP 探針</th>
        </tr>
    </table>
<!--服務器相關參數-->
<table>
  <tr><th colspan="4">服務器參數</th></tr>
  <tr>
    <td>服務器域名/IP地址</td>
  ...
  <tr>
    <td width="13%">服務器操作系統</td>
    <td width="37%"><?php $os = explode(" ", php_uname()); echo $os[0];?> &nbsp;內核版本：<?php if('/'==DIRECTORY_SEPARATOR){echo $os[2];}else{echo $os[1];} ?></td>
    <td width="13%">服務器解譯引擎</td>
...
<?if("show"==$sysReShow){?>
<table>
  <tr><th colspan="6">服務器實時數據</th></tr>

  <tr>
    <td width="13%" >服務器當前時間</td>
    <td width="37%" ><span id="stime"><?php echo $stime;?></span></td>
    <td width="13%" >服務器已運行時間</td>
    <td width="37%" colspan="3"><span id="uptime"><?php echo $uptime;?></span></td>
  </tr>
  <tr>
    <td width="13%">CPU 型號 [<?php echo $sysInfo['cpu']['num'];?>核]</td>
...
{
?>        
          Cache化內存為 <span id="CachedMemory"><?php echo $mc;?></span>
          , 使用率 
          <span id="memCachedPercent"><?php echo $memCachedPercent;?></span>
          %    | Buffers緩衝為  <span id="Buffers"><?php echo $mb;?></span>
...
<a name="w_php"></a>
<table>
  <tr><th colspan="4">PHP 相關參數</th></tr>
  <tr>
  ...
  <tr>
    ...
    <td>socket 超時時間(default_socket_timeout)</td>
    <td><?php echo show("default_socket_timeout");?>秒</td>
  </tr>

  <tr>
    <td>PHP 頁面根目錄(doc_root)</td>
  ...
  <tr>
    <td>數據反斜槓轉義(magic_quotes_gpc)</td>
  ...
  <tr>
    <td>"&lt;% %&gt;"ASP 風格標記(asp_tags)</td>
    <td><?php echo show("asp_tags");?></td>
    <td>忽略重複錯誤信息(ignore_repeated_errors)</td>
  ...
  <tr>
    ...
    <td>報告內存洩漏(report_memleaks)</td>
    <td><?php echo show("report_memleaks");?></td>
  </tr>

  <tr>
    <td>自動字符串轉義(magic_quotes_gpc)</td>
    <td><?php echo show("magic_quotes_gpc");?></td>
    <td>外部字符串自動轉義(magic_quotes_runtime)</td>
  ...
  <tr>
    ...
    <td>聲明 argv 和 argc 變量(register_argc_argv)</td>
...
<!--MySQL數據庫連接檢測-->
<table>
  <tr><th colspan="3">MySQL 數據庫連接檢測</th></tr>
...
<?php
  if ($_POST['act'] == 'MySQL檢測') {
      if(function_exists("mysql_close")==1) {
          ...
          if ($link){
              echo "<script>alert('連接到MySql數據庫正常')</script>";
          } else {
              echo "<script>alert('無法連接到MySql數據庫！')</script>";
          }
      } else {
          echo "<script>alert('服務器不支持MySQL數據庫！')</script>";
      }
  }
?>
...
?>
    ...
    <table>
        <tr>
            <td class="w_foot"><a href="http://lamp.myxnova.com/" target="_blank">基於 YaHei.net 探針</a></td>
            ...
            <td class="w_foot"><a href="#w_top">返回頂部</a></td>
        </tr>
    </table>
</div>
</body>
