...
if [ -s pure-ftpd-1.0.36.tar.gz ]; then
    ...
    if ! wget -c http://lampd.googlecode.com/svn/src/pure-ftpd-1.0.36.tar.gz;then
...
if ! wget --no-check-certificate http://lampd.googlecode.com/svn/src/pure-ftpd.conf -O /etc/pure-ftpd.conf;then
